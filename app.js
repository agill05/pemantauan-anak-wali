...
async function renderDashboard() {
    showLoading("Memuat statistik dashboard...");
...
async function saveKeagamaanForm(e, id) {
    e.preventDefault();
    const siswa_id = document.getElementById("m-kag-siswa").value;
    const nama_surat = document.getElementById("m-kag-surah").value;
    const tanggal = document.getElementById("m-kag-tanggal").value;
    const status = document.getElementById("m-kag-status").value;
    const catatan = document.getElementById("m-kag-catatan").value;

    const res = await apiCall("saveKeagamaan", { id: id || null, siswa_id, nama_surat, tanggal, status, catatan }, true);
    if (res && res.status === "success") {
        closeModal();
